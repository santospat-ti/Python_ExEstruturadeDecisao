"""As Organizações Tabajara resolveram dar um aumento de salário aos seus colaboradores e lhe contraram para desenvolver 
o programa que calculará os reajustes.
Faça um programa que recebe o salário de um colaborador e o reajuste segundo o seguinte critério, baseado no salário atual:
salários até R$ 280,00 (incluindo) : aumento de 20%
salários entre R$ 280,00 e R$ 700,00 : aumento de 15%
salários entre R$ 700,00 e R$ 1500,00 : aumento de 10%
salários de R$ 1500,00 em diante : aumento de 5% Após o aumento ser realizado, informe na tela:
o salário antes do reajuste;
o percentual de aumento aplicado;
o valor do aumento;
o novo salário, após o aumento."""

print('\tORGANIZAÇÕES TABAJARA')
salario = float(input('Digite seu salário: R$ '))
reajuste = ''
resultado = ''
print(f'Seu salário antes do reajuste era R$ {salario:.2f}')

if salario <= 280.00:
    reajuste = (20 / 100) * salario
    resultado = salario + reajuste
    print(f'Percentual aumentado: 20%')
    print(f'O valor do aumento é R$ {reajuste:.2f}')

elif salario > 280.00 and salario <= 700.00:
    reajuste = (15 / 100) * salario
    resultado = salario + reajuste
    print('Percentual aumentado: 15%')
    print(f'O valor do aumento é de R${reajuste:.2f}')

elif salario > 700.00 and salario <= 1500.00:
    reajuste = (10 / 100) * salario
    resultado = salario + reajuste
    print('Percentual aumentado: 10%')
    print(f'O valor do aumento é de R${reajuste:.2f}')

elif salario > 1500.00:
    reajuste = (5 / 100) * salario
    resultado = salario + reajuste
    print('Percentual aumentado: 5%')
    print(f'O valor do aumento é de R${reajuste:.2f}')
    
print(f'Seu salário depois do reajuste é R$ {resultado:.2f}')

